...
from Customers as c
    ...
    on od.OrderID = o.OrderID
where year(OrderDate) in('1996', '1998');
--Mostrar el nombre del producto, el nombre del proveedor y el precio unitario de cada producto.
...
--Listar el nombre del cliente, el ID del pedido y la fecha del pedido para cada pedido.
...
--Obtener el nombre del empleado, el título del cargo y el departamento del empleado para cada empleado.
select CONCAT(e.FirstName,' ',e.LastName) as 'NombreCopmleto', e.TitleOfCourtesy, e.Address
from Employees as e;
-- Mostrar el nombre del proveedor, el nombre del contacto y el teléfono del contacto para cada proveedor.
select CompanyName, ContactName, Phone
from Suppliers
--Listar el nombre del producto, la categoría del producto y el nombre del proveedor para cada producto.
...
--Obtener el nombre del producto, el nombre de la categoría y la descripción de la categoría para cada producto que pertenece a una categoría.
select p.ProductName as 'NombreProducto', c.CategoryName as 'NombreCategoría', c.Description as 'DescripciónCategiría'
from Products as p
    inner join Categories as c
    on c.CategoryID = p.CategoryID;
--Seleccionar el total de ordenes hechas por cada uno de los proveedores 
...
--Seleccionar el total de dinero que ha vendido por proveedor el ultimo trimestre de 1996
